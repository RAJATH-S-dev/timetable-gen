...
import { signUpDepartment } from '@/lib/supabase/actions'
...
export default function RegisterPage() {
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    setIsLoading(true)
    try {
      await signUpDepartment(email, password)
  ...
  return (
    <div style={{
      ...
      <style>{`
        ...
        .register-card { animation: fadeIn 0.5s ease-out both; }
        .register-input:focus { border-color: #2D3436 !important; }
      `}</style>
      ...
      <div className="register-card" style={{ width: '100%', maxWidth: 420 }}>
        ...
        <div style={{
          ...
          <div style={{ textAlign: 'center', marginBottom: 28 }}>
            ...
            <p style={{ fontSize: 13, color: '#8B7D6B', margin: 0 }}>
              Create an admin account for your department
            </p>
          </div>
        ...
        <p style={{ textAlign: 'center', fontSize: 12, color: '#8B7D6B', marginTop: 20 }}>
          ...
          <Link href="/login" style={{ color: '#2D3436', fontWeight: 700, textDecoration: 'none' }}>
            Sign in
          </Link>
        </p>
        ...
        {/* Institution note */}
        <p style={{
          textAlign: 'center', fontSize: 10, color: '#B0A898',
